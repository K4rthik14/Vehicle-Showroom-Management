...
<div class="row g-3">
    ...
    <div class="col-lg-4">
        <div class="card-vsms">
            <div class="card-header"><i class="bi-plus-circle me-2 text-primary"></i><?= $editData ? 'Edit Vehicle' : 'Add Vehicle' ?></div>
            <div class="card-body">
                <form method="POST">
                    <?php if ($editData): ?>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select">
                            <option <?= ($editData['Status']??'Available')==='Available'?'selected':'' ?>>Available</option>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn-primary-vsms btn"><?= $editData ? 'Update' : 'Add Vehicle' ?></button>
                        <?php if ($editData): ?>
    ...
    <div class="col-lg-8">
        <div class="card-vsms">
            <div class="card-header"><i class="bi-car-front me-2 text-primary"></i>All Vehicles (<?= $vehicles->num_rows ?>)</div>
            <div class="card-body p-0">
                <?php if ($vehicles->num_rows > 0): ?>
                <table class="table table-vsms mb-0">
                    <thead><tr><th>VIN</th><th>Model</th><th>Fuel</th><th>Status</th><th>Qty</th><th>Supplier</th><th>Actions</th></tr></thead>
                    <tbody>
                    <?php while ($v = $vehicles->fetch_assoc()): ?>
                        <tr>
                            <td><span style="font-family:'DM Mono',monospace;font-size:0.78rem;"><?= htmlspecialchars(substr($v['VIN'],0,14)).'...' ?></span></td>
                            <td><strong><?= htmlspecialchars($v['Model_Name']) ?></strong></td>
                            <td><?= $v['Fuel_type'] ?></td>
                            <td><span class="badge badge-<?= strtolower($v['Status']) ?>"><?= $v['Status'] ?></span></td>
                            <td><?= $v['Stock_Quantity'] ?></td>
                            <td><?= htmlspecialchars($v['S_Name'] ?? '—') ?></td>
                            <td>
                                <a href="vehicles.php?edit=<?= urlencode($v['VIN']) ?>" class="btn btn-sm btn-outline-secondary me-1">
                                    <i class="bi-pencil"></i>
                                </a>
                                <?php if (isAdmin()): ?>
                                <a href="vehicles.php?delete=<?= urlencode($v['VIN']) ?>" class="btn btn-sm btn-outline-danger"
                ...
                <?php else: ?>
                    <div class="empty-state"><i class="bi-car-front"></i><p>No vehicles found.</p></div>
                <?php endif; ?>
            </div>
        </div>
    </div>
