...
<div class="row g-3">
    <div class="col-lg-4">
        <div class="card-vsms">
            <div class="card-header"><i class="bi-plus-circle me-2 text-primary"></i><?= $editSale ? 'Edit' : 'Record' ?> Sale</div>
            <div class="card-body">
                <form method="POST">
                    ...
                    <div class="mb-2">
                        ...
                        <select name="vin" class="form-select" required>
                            ...
                            ?>
                                <option value="<?= htmlspecialchars($vehicle['VIN']) ?>"
                                    <?= ($editSale['VIN'] ?? '') === $vehicle['VIN'] ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($vehicle['Model_Name']) ?> (<?= substr($vehicle['VIN'], -6) ?>)
                                </option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <select name="customer_id" class="form-select" required>
                            ...
                            <?php $customers->data_seek(0); while ($customer = $customers->fetch_assoc()): ?>
                                <option value="<?= $customer['Customer_ID'] ?>" <?= ($editSale['Customer_ID'] ?? '') == $customer['Customer_ID'] ? 'selected' : '' ?>>
                    ...
                    <div class="mb-2">
                        ...
                        <select name="emp_id" class="form-select" required>
                            ...
                            <?php $staffMembers->data_seek(0); while ($staff = $staffMembers->fetch_assoc()): ?>
                                <option value="<?= $staff['Emp_ID'] ?>" <?= ($editSale['Emp_ID'] ?? '') == $staff['Emp_ID'] ? 'selected' : '' ?>>
                    ...
                    <div class="mb-2">
                        ...
                        <input type="date" name="sale_date" class="form-control" required value="<?= $editSale['Sale_Date'] ?? date('Y-m-d') ?>">
                    </div>

                    <div class="mb-2">
                        <label class="form-label">Amount (Rs.) *</label>
                        <input type="number" name="amount" class="form-control" min="0" step="0.01" required value="<?= $editSale['Amount'] ?? '' ?>">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="payment_method" class="form-select">
                            <?php foreach (['Cash', 'Card', 'UPI'] as $method): ?>
                                <option <?= ($editSale['Payment_Method'] ?? 'Cash') === $method ? 'selected' : '' ?>><?= $method ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>

                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary-vsms"><?= $editSale ? 'Update' : 'Record Sale' ?></button>
                        <?php if ($editSale): ?><a href="sales.php" class="btn btn-outline-vsms">Cancel</a><?php endif; ?>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="col-lg-8">
        <div class="card-vsms">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-success" style="font-size:.8rem;">Total: Rs.<?= number_format($totalRevenue) ?></span>
            </div>
            <div class="card-body p-0">
                ...
                <table class="table table-vsms mb-0">
                    ...
                    <?php while ($sale = $sales->fetch_assoc()): ?>
                        <tr>
                            <td style="font-family:'DM Mono',monospace;color:#0f766e;"><?= $sale['Sale_ID'] ?></td>
                            ...
                            <td>
                                <a href="sales.php?edit=<?= $sale['Sale_ID'] ?>" class="btn btn-sm btn-outline-secondary me-1"><i class="bi-pencil"></i></a>
                                <?php if (isAdmin()): ?>
                                <a href="sales.php?delete=<?= $sale['Sale_ID'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete sale and restore vehicle?')"><i class="bi-trash"></i></a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endwhile; ?>
